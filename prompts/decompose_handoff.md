...
- Up to **{{decompose_items_max}} items** total across columns; prefer the most actionable **visible** items under the cap — **only from the snapshot**, not a guessed full mailbox/feed.
- **Homogeneous clump (preferred when safe):** if several **visible** rows share the **same agent pattern** (same site + same open→observe→verify/read class, **no** divergent human remainder such as reply vs decide vs apply), emit **one Agent root** that covers the set (title names the set; optional `hints` with senders/subjects or a covering `search_query`). Prefer **1–2** Agent roots total when the viewport is one homogeneous pattern class — not N near-identical cards.
- **Split cards** when goals diverge (different human actions or different patterns). Dense **one item per clearly distinct visible row** only when rows are **not** homogeneous under the clump rule. Cap remains `decompose_items_max`, not “one card = one email forever.”
