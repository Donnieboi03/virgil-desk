...
- **Path B (default):** DOM **Eyes** (slim `interact_targets`, optional AX/`page_tree`, probes) → **Hands** by `target_id` — see [`BROWSER_LAYER.md`](BROWSER_LAYER.md) and [`INTERACTION_LAYERS.md`](INTERACTION_LAYERS.md)
...
5. **You column** — **Mark done** when human closes the loop (auth walls / soft-help park here)
